    ...
    <main>
      ...
      <section className="relative overflow-hidden pt-[148px] md:pt-[160px]">
        ...
        <div className="section-shell relative pb-10 md:pb-14">
          ...
          <h1 className="mt-3 max-w-3xl text-4xl leading-tight md:text-6xl">Track Your Order</h1>
      ...
      <section className="pb-16 pt-2">
        <div className="section-shell max-w-xl">
          <form
            ...
            className="relative overflow-hidden rounded-2xl border border-[#ffffff20] bg-gradient-to-br from-[#4a0d15]/90 via-[#3a0b12]/85 to-[#2d070e]/90 p-7 shadow-[0_16px_38px_rgba(0,0,0,0.32)]"
          >
            ...
            <div className="mb-5">
              ...
              <input
                ...
                required
                className="w-full rounded-xl border border-[var(--gold)]/30 bg-[#2d0a10] px-4 py-3 text-sm text-white outline-none transition placeholder:text-[#8a6360] focus:border-[var(--gold)]"
              />
            </div>
            <div className="mb-6">
          ...
          {lookupError ? (
            <div className="relative overflow-hidden rounded-2xl border border-[var(--gold)]/30 bg-gradient-to-br from-[#4a0d15]/90 via-[#3a0b12]/85 to-[#2d070e]/90 p-8 text-center shadow-[0_16px_38px_rgba(0,0,0,0.32)]">
              <PackageSearch className="mx-auto mb-4 h-12 w-12 text-[var(--gold)]/60" />
              <h2 className="text-2xl text-[#fff4ef]">Order Not Found</h2>
              <p className="mt-3 text-sm leading-7 text-[#f2d8cf]">
          ...
          {order ? (
            <div className="mt-6 rounded-2xl border border-[var(--gold)]/30 bg-gradient-to-br from-[#4a0d15]/90 via-[#3a0b12]/85 to-[#2d070e]/90 p-6 shadow-[0_16px_38px_rgba(0,0,0,0.32)]">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#ffffff20] pb-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-[var(--gold)]">Order ID</p>
                  <p className="mt-1 text-lg text-white">{order.id}</p>
                </div>
              ...
              <div className="mt-4 grid gap-3 text-sm text-[#f2d8cf] sm:grid-cols-3">
                <p>
                  <span className="text-[#caa69f]">Placed on:</span> {new Date(order.createdAt).toLocaleString()}
                </p>
                <p>
                  <span className="text-[#caa69f]">Payment:</span> {order.paymentMethod === "cod" ? "Cash on Delivery" : "Online"}
                </p>
                <p>
                  <span className="text-[#caa69f]">Total:</span> {order.currencyCode} {order.totalAmount}
                </p>
              </div>

              <div className="mt-5 space-y-3">
                {order.items.map((item) => (
                  <div key={`${order.id}-${item.productId}`} className="flex items-center justify-between rounded-xl border border-[#ffffff18] bg-[#34090f]/60 px-4 py-3 text-sm">
                    <div>
                      <p className="text-white">{item.name}</p>
                      <p className="text-xs text-[#caa69f]">Qty: {item.quantity}</p>
                    </div>
          ...
          <div className="mt-6 rounded-2xl border border-[#ffffff14] bg-[#3a0b12]/50 p-5 text-sm text-[#e9cfc4]">
            ...
            <p className="mt-1 leading-6">
              ...
              <span className="font-medium text-white">My Account → Orders</span> for real-time status.
            </p>
          </div>
        </div>
      </section>
