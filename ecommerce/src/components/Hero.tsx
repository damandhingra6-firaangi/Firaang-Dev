...
const BASE_HERO_SLIDE: HeroSlide = {
  ...
  subtitle: "Premium Clothing And Jewellery, Sculpted For Those Who Treat Their Wardrobe Like An Archive.",
...
function mapFeaturedCollectionToSlide(collection: ShopifyCollectionLaunch): HeroSlide {
  ...
  return {
    ...
    subtitle: collection.launchSubtitle || collection.description || "Explore the latest limited campaign from Firaang.",
...
export default function Hero({ featuredCollections = [] }: HeroProps) {
  const slides = useMemo(
    () => [BASE_HERO_SLIDE, ...featuredCollections.map(mapFeaturedCollectionToSlide)],
    [featuredCollections],
  );
